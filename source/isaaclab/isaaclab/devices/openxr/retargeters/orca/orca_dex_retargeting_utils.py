...
# in Dex-retargeting. We keep the standard 21-point MANO indexing so the
# DexPilot configuration can select the subset it needs.
...
class OrcaDexRetargeting:
    """A class for Orca hand retargeting.

    Handles retargeting of OpenX hand tracking data to Orca robot hand joint angles.
    """

    def __init__(
        self,
        hand_joint_names: list[str],
        right_hand_config_filename: str = "orca_hand_right_dexpilot.yml",
        left_hand_config_filename: str = "orca_hand_left_dexpilot.yml",
        left_hand_urdf_path: str = "source/isaaclab_assets/data/robot/orca/urdf/orcahand_left.urdf",
        right_hand_urdf_path: str = "source/isaaclab_assets/data/robot/orca/urdf/orcahand_right.urdf",
    ):
        ...
        omni.log.info("[OrcaRetargeting] init done.")
    ...
    def compute_left(self, left_hand_poses: dict[str, np.ndarray]) -> np.ndarray:
        ...
        if left_hand_poses is not None:
            ...
            left_hand_q = np.zeros(len(self.left_dof_names))
        return left_hand_q
    ...
    def compute_right(self, right_hand_poses: dict[str, np.ndarray]) -> np.ndarray:
        ...
        if right_hand_poses is not None:
            ...
            right_hand_q = np.zeros(len(self.right_dof_names))
        return right_hand_q
